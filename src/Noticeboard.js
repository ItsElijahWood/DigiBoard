...
function Noticeboard() {
  ...
  useEffect(() => {
    async function getIdAndFetchConfig() {
      ...
      const response = await fetch("/api/noticeboard/id/from-url", {
        method: 'POST',
        headers: {
      ...
      const response2 = await fetch('/api/noticeboard/config/fetch_1', {
        method: "POST",
        headers: {
          'Content-Type': 'application/json'
        },
      ...
      async function fetchContent() {
        const response = await fetch('/api/noticeboard/fetch_1', {
          method: "POST",
          headers: {
            'Content-Type': 'application/json'
          },
        ...
        if (response.ok) {
          ...
          for (const file of files[0]) {
            const response2 = await fetch('/api/noticeboard/fetchfile_1', {
              method: "POST",
              headers: {
                'Content-Type': 'application/json'
              },
  ...
  return (
    <>
      <div className="header_noticeboard_1" style={{ padding: "30px", display: "flex", justifyContent: "space-between" }}>
        <p className="title-noticeboard_1" style={{ color: textColour }}>{title}</p>
        <div style={{ display: "flex" }}>
          ...
          {
            buttonToggle3 && (
              <a className="buttons-a" style={{ color: `${textColour}` }} href={`/noticeboard_3?v=${buttonsUrls3.n3}`}>{title3}</a>
            )
          }
          {
            buttonToggle4 && (
          ...
          {files.map((file, idx) => (
            <div key={idx} className="file" ref={divRef}>
              <img
                ...
                src={file.data}
                title={file.name}
                style={{ userSelect: "none", border: "0px solid #ffffff", pointerEvents: "none" }}
                onLoad={(e) => {
                  const img = e.target;
