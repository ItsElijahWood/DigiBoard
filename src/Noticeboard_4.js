...
function Noticeboard_4() {
  ...
  useEffect(() => {
    async function getIdAndFetchConfig() {
      ...
      const response = await fetch("/api/noticeboard/id/from-url_4", {
        method: 'POST',
        headers: {
      ...
      const response2 = await fetch('/api/noticeboard/config/fetch_4', {
        method: "POST",
        headers: {
          'Content-Type': 'application/json'
        },
      ...
      async function fetchContent() {
        const response = await fetch('/api/noticeboard/fetch_4', {
          method: "POST",
          headers: {
            'Content-Type': 'application/json'
          },
        ...
        if (response.ok) {
          ...
          for (const file of files[0]) {
            const response2 = await fetch('/api/noticeboard/fetchfile_4', {
              method: "POST",
              headers: {
                'Content-Type': 'application/json'
              },
  ...
  return (
    <>
      <div className="header_noticeboard_1" style={{ padding: "30px", display: "flex", justifyContent: "space-between" }}>
        <p className="title-noticeboard_1" style={{ color: textColour }}>{title4}</p>
